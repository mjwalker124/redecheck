...
public class SmallRangeFailure extends ResponsiveLayoutFailure {
  ...
  public SmallRangeFailure(
      AlignmentConstraint ac, AlignmentConstraint prev, AlignmentConstraint next) {
    this.ac = ac;
    this.prev = prev;
    this.next = next;
  }
  ...
  public void captureScreenshotExample(
    ...
    ////                    splits[1];
    //            File output = new File(new java.io.File( "." ).getCanonicalPath() + "/../reports/"
    //  + webpage + "/" + mutant + "/fault"+errorID+"/");
    //            FileUtils.forceMkdir(output);
    //            ImageIO.write(img, "png", new File(output+ "/prev-Width" + captureWidth +
    // ".png"));
    //        } catch (IOException e) {
    //            e.printStackTrace();
    //        }
    ...
    //  + webpage + "/" + mutant + "/fault"+errorID+"/");
    //            FileUtils.forceMkdir(output);
    //            ImageIO.write(img, "png", new File(output+ "/next-Width" + captureWidth +
    // ".png"));
    //        } catch (IOException e) {
    //            e.printStackTrace();
    //        }
