...
public class AGDiff {
  ...
  public AGDiff(Map<DomNode, DomNode> matchedNodes, AlignmentGraph ag1, AlignmentGraph ag2) {
  ...
  private void compareParents(AGNode a, AGNode b, List<String> issues) {
    ...
    if (c1 != null && c2 != null) {
      ...
      if (expected_p2 != null && !StringUtils.equals(expected_p2.getxPath(), p2.getxPath())) {
  ...
  private String errMsg(String msg, AGEdge x, AGEdge y) {
    ...
    return String.format("\"%s\",\"(%s-%s)\",\"(%s-%s)\"", msg, xp1a, xp1b, xp2a, xp2b);
  }
  ...
  private boolean isSignificantDiff(int a, int b, int c, int d) {
    ...
    return Math.abs(x - y) > diffThreshold;
  }
