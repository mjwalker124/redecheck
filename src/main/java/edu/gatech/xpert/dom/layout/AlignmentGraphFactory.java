...
public class AlignmentGraphFactory {
  ...
  public static String generateEdgeLabelling(AGEdge e) {
    ...
    if (e instanceof Sibling) {
      ...
      if (c.isCentered()) result += "centered";
      if (c.isLeftJustified()) result += "leftJust";
      if (c.isRightJustified()) result += "rightJust";
      if (c.isMiddle()) result += "middle";
      if (c.isTopAligned()) result += "top";
      if (c.isBottomAligned()) result += "bottom";
    }
    return result;
  }
  ...
  public static boolean isAlignmentTheSame(AGEdge a, AGEdge b) {
    if (a instanceof Contains) {
      ...
      if (c1.isTopAligned() != c2.isTopAligned()) {
        ...
        return false;
      } else return c1.isCentered() == c2.isCentered();
    } else {
      ...
      if (a.getNode1().getxPath().equals(b.getNode1().getxPath())) {
        if (s1.isBottomTop() != s2.isBottomTop()) {
          ...
          return false;
        } else return s1.isBottomEdgeAligned() == s2.isBottomEdgeAligned();
      } else {
  ...
  public static String generateKey(AGEdge e) {
    ...
    if (e instanceof Contains)
      return e.getNode1().getxPath()
          + e.getNode2().getxPath()
          + "contains"
          + generateEdgeLabelling(e);
    else
      return e.getNode1().getxPath()
          + e.getNode2().getxPath()
          + "sibling"
    ...
    for (AGNode n : nodeMapCopy.values()) {
      try {
        ...
        if ((Arrays.equals(dn.getCoords(), parent.domNode.getCoords())) && onlyChild) {
          ...
          //
          String key =
              oldContains.getChild().domNode.getxPath()
                  + oldContains.getParent().domNode.getxPath()
                  + "contains"
                  + generateEdgeLabelling(oldContains);
          ...
          for (Contains cEdge : childrenEdges) {
            ...
            this.edgeMap.put(
                newContains.getChild().domNode.getxPath()
                    + newContains.getParent().domNode.getxPath()
                    + "contains"
                    + generateEdgeLabelling(newContains),
                newContains);
            ...
            String newKey =
                cEdge.getChild().domNode.getxPath()
                    + cEdge.getParent().domNode.getxPath()
                    + "contains"
                    + generateEdgeLabelling(cEdge);
  ...
  private HashMap<String, AGEdge> generateEdgeMap() {
    ...
    for (Contains c : ag.contains) {
      edgeMap.put(
          c.getNode1().getxPath() + c.getNode2().getxPath() + "contains" + generateEdgeLabelling(c),
          c);
    ...
    for (Sibling s : ag.siblings) {
      edgeMap.put(
          s.getNode1().getxPath() + s.getNode2().getxPath() + "sibling" + generateEdgeLabelling(s),
          s);
    }
    return edgeMap;
  }
