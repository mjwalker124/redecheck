...
public class ParentChild extends Relationship {
  ...
  private void assignAttributes() {
    ...
    if (equals(px, cx, dW_delta)
      ...
      if (equals(px, cx, dW)) {
        ...
        //                if
        // (child.getXpath().equals("/HTML/BODY/DIV[3]/DIV/DIV[2]/DIV/DIV/DIV/DIV[9]")) {
        //                    System.out.println(mLeft + " " + mRight);
        //                }
        //                if (mLeft.equals("0px") && mRight.equals("0px")) {
        ////                    System.out.println(child.getXpath() + " was intentionally
        // centralized");
        //                } else {
        //                    System.out.println(child.getXpath() + " was coincidentally
        // centralized");
        //                }
    ...
    if (equals(py, cy, dH_delta)
        && equals(child.y1, parent.y1, dH_delta)
  ...
  private String generateAttributeSet() {
    String result = " {";
    if (isCentreJust()) result += "centered";
    if (isLeftJust()) result += "leftJust";
    if (isRightJust()) result += "rightJust";
    if (isMiddleJust()) result += "middle";
    if (isTopJust()) result += "top";
    if (isBottomJust()) result += "bottom";
