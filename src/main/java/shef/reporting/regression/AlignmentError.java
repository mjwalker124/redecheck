...
/** Created by thomaswalsh on 26/08/15. Last modified on 15/09/15 */
public class AlignmentError extends Error implements Comparable<AlignmentError> {
  ...
  public AlignmentConstraint getOracle() {
    return oracle;
  }

  public AlignmentConstraint getTest() {
    return test;
  }
    ...
    switch (desc) {
      ...
      case "diffBounds":
        ...
        if ((oracle.getMin() != test.getMin()) && (oracle.getMax() == test.getMax())) {
          errorRanges.add(
              new int[] {
                Math.min(oracle.getMin(), test.getMin()),
                Math.max(oracle.getMin(), test.getMin()) - 1
              });
        } else if ((oracle.getMin() == test.getMin()) && (oracle.getMax() != test.getMax())) {
          errorRanges.add(
              new int[] {
                Math.min(oracle.getMax(), test.getMax()) + 1,
                Math.max(oracle.getMax(), test.getMax())
              });
        } else if ((oracle.getMin() != test.getMin()) && (oracle.getMax() != test.getMax())) {
          errorRanges.add(
              new int[] {
