...
public class Layout {
  ...
  public Layout(
      RTree rtree,
      HashMap<Integer, Rectangle> rects,
      HashMap<Integer, String> xs,
      HashMap<String, Element> els) {
    this.rtree = rtree;
  ...
  private void extract() {
      ...
      for (int i : cIDs) {
        ...
        if (parentMap.containsKey(i)) {
          ...
          if (tempIds.contains(c)) {
            //                    if (rectangles.get(c).area() <
            // rectangles.get(currentParent).area()) {
            //                        if
            // (xpaths.get(i).equals("/HTML/BODY/DIV[2]/DIV[2]/DIV[2]/FORM/DIV/INPUT")) {
            //                            System.out.println("Changing parent map from " +
            // xpaths.get(currentParent) + " to " + xpaths.get(i) + " and " + xpaths.get(c));
            //                        }
            parentMap.put(i, c);
          } else {
            //                        if
            // (xpaths.get(i).equals("/HTML/BODY/DIV[2]/DIV[2]/DIV[2]/FORM/DIV/INPUT")) {
            //                            System.out.println("Didn't change parent map with " +
            // xpaths.get(i) + " and " + xpaths.get(c));
            //                            System.out.println(xpaths.get(currentParent) + " " +
            // rectangles.get(currentParent));
            //                            System.out.println(xpaths.get(c) + " " +
            // rectangles.get(c));
            //                            System.out.println();
            //                        }
          }
    ...
    //                                System.out.println(xpaths.get(x) + " child of " +
    // xpaths.get(parentMap.get(x)));
    //                            }
    //                        }
    //                    }
    //                }
