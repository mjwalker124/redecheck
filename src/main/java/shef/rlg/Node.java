...
public class Node {
  ...
  /**
   * Returns the parent-child alignment constraints for which the node is the child currently linked
   * to the node.
   *
  ...
  /**
   * Generates an informative string displaying the node's XPath and width constraints, which can
   * then be printed
   *
  ...
  /**
   * Generates a string for printing the node's information into a GraphViz node, to illustrate the
   * node visually.
   *
   ...
   */
  public void addParentConstraint(AlignmentConstraint ac) {
    parentConstraints.add(ac);
  }
