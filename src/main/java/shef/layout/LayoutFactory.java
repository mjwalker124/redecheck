...
public class LayoutFactory {
    ...
    public static String generateEdgeLabelling(Relationship r) {
        ...
        if (r instanceof Sibling) {
            ...
            if (pc.isCentreJust()) result += "centered";
            if (pc.isLeftJust()) result += "leftJust";
            if (pc.isRightJust()) result += "rightJust";
            if (pc.isMiddleJust()) result += "middle";
            if (pc.isTopJust()) result += "top";
            if (pc.isBottomJust()) result += "bottom";
        }
        return result + "}";
    }
    ...
    public static String generateKey(Relationship r) {
        ...
        if (r instanceof ParentChild)
            return r.getNode1().getXpath()
                    + " contains "
                    + r.getNode2().getXpath()
                    + generateEdgeLabelling(r);
        else
            return r.getNode1().getXpath()
                    + " sibling of "
                    + r.getNode2().getXpath()
    ...
    public Element getElementFromDomData(JSONObject obj) throws JSONException {
        ...
        Boolean inHead = xpath.contains("head") || xpath.contains("HEAD");
        if (!Arrays.equals(new int[] {0, 0, 0, 0}, coords) || inHead) {
            try {
                ...
                if (!(((width < 1) || (height < 1)) && (obj.getInt("overflow") == 0)) || inHead) {
                    if (obj.getInt("visible") == 1 || inHead) {
                        if (height >= 5 && width >= 5 || inHead) {
                            if (!ArrayUtils.contains(tagsIgnore, parseTagName(xpath).toUpperCase())) {
                                if (!childOfNoSizeElement(xpath) || inHead) {
                                    try {
                                    ...
                                    //                                System.out.println(xpath + " was a child of a
                                    // no-size or non-visible element");
                                }
                            }
                        } else {
                            //                            System.out.println(xpath + " was too small with size " +
                            // width + " x " + height);
                        }
