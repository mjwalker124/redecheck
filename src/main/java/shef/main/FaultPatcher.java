...
public class FaultPatcher {
  ...
  public FaultPatcher(String fullUrl, String url, String browser, String current) {
    ...
    try {
      ...
      directory =
          ResultProcessor.lastFileModified(current + "/../reports/" + url.split("/")[0] + "/");
      ...
      for (int i = 0; i < errors.length; i++) {
        if (classifications[i].equals("TP")) {
          ...
          int originalFitness =
              calculateFitness(originalLF, nodes, categories[i], error, fBounds[1]);
          ...
          for (int x = 1; x < 3; x++) {
            ...
            while (!worklist.isEmpty()) {
              ...
              if (currentFitness == 1000) {
                ...
                currentFitness =
                    calculateFitness(currentLF, nodes, categories[i], error, fBounds[1]);
                System.out.println("Changed fitness to " + currentFitness);
              }

              //                            currentCM.writeToFile(0, currentCM.getStylesheets(),
              // mutator.getShorthand(), newUrl);
              //                            webDriver.get(newUrl + "/index.html");
              //                            checkForFailure(nodes, categories[i], currentSize,
              // error);
              //                            oldDomString = domStrings.get(currentSize);
              //                            oldLF = new LayoutFactory(oldDomString);
              //                            oldFitness = calculateFitness(oldLF, nodes,
              // categories[i], error, fBounds[1]);
              //                            System.out.println("OLD: " + oldFitness);

              HashMap<String, CSSMutator> options =
                  currentCM.getMutators(currentCM.getStylesheets());
  ...
  private void resizeBrowserUntilFaulty(
      boolean failureManifesting, int currentSize, String error, int i, ArrayList<String> nodes) {
  ...
  private String generateRandomInjectionScript(ArrayList<String> nodes, String error) {
    String result =
        "var element = document.evaluate(\""
            + nodes.get(0)
            + "\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; var styles = window.getComputedStyle(element);  element.style.fontSize = '20px';";
    return result;
  }

  private int calculateFitness(
      LayoutFactory lf, ArrayList<String> nodes, String category, String error, int fBound) {
  ...
  private boolean checkForFailure(
      ArrayList<String> nodes, String category, int fBound, String error) {
    ...
    if (category.equals("Element Collision") || category.equals("Element Protrusion")) {
      ...
      for (Relationship r : layout.getRelationships().values()) {
        if (r instanceof Sibling) {
          ...
          } else if (s.getNode2().getXpath().equals(wrapped)
              && nodes.contains(s.getNode1().getXpath())) {
