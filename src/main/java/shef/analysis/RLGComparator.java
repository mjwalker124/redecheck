...
public class RLGComparator {
  ...
  /**
   * Takes a set of matched nodes from the two RLG models and compares all the different constraints
   * on them, producing a list of model differences.
   *
  ...
  /**
   * Matches the nodes from the oracle version to those in the test version, so their constraints
  ...
  public void compareAlignmentConstraints(Node n, Node m) {
    ArrayList<AlignmentConstraint> ac1 = new ArrayList<AlignmentConstraint>(),
        ac2 = new ArrayList<AlignmentConstraint>();
    ArrayList<AlignmentConstraint> ac1f = new ArrayList<AlignmentConstraint>(),
        ac2f = new ArrayList<AlignmentConstraint>();
    ...
    HashMap<AlignmentConstraint, AlignmentConstraint> matched =
        new HashMap<AlignmentConstraint, AlignmentConstraint>();
    ArrayList<AlignmentConstraint> unmatched1 = new ArrayList<AlignmentConstraint>(),
        unmatched2 = new ArrayList<AlignmentConstraint>();
    while (ac1.size() > 0) {
      AlignmentConstraint ac = ac1.remove(0);
      AlignmentConstraint match = null;
      for (AlignmentConstraint temp : ac2) {
        if ((temp.node1.getXpath().equals(ac.node1.getXpath()))
            && (temp.node2.getXpath().equals(ac.node2.getXpath()))) {
          if ((temp.getMin() == ac.getMin())
              && (temp.getMax() == ac.getMax())
              && (Arrays.equals(ac.getAttributes(), temp.getAttributes()))) {
            match = temp;
          } else if ((temp.getMin() == ac.getMin())
              && (temp.getMax() == ac.getMax())
              && (!Arrays.equals(ac.getAttributes(), temp.getAttributes()))) {
            AlignmentError ae = new AlignmentError(ac, temp, "diffAttributes");
            acErrors.add(ae);
            match = temp;
          } else if ((Arrays.equals(ac.getAttributes(), temp.getAttributes()))
              && ((temp.getMin() != ac.getMin()) || (temp.getMax() != ac.getMax()))) {
      ...
      if (match != null) {
        ...
        for (AlignmentConstraint f : ac2f) {
          if ((ac.node1.getXpath().equals(f.node2.getXpath()))
              && (ac.node2.getXpath().equals(f.node1.getXpath()))) {
            if ((f.getMin() == ac.getMin())
                && (f.getMax() == ac.getMax())
                && (ac.generateLabelling().equals(f.generateFlippedLabelling()))) {
              match = f;
            } else if ((f.getMin() == ac.getMin())
                && (f.getMax() == ac.getMax())
                && (!ac.generateLabelling().equals(f.generateFlippedLabelling()))) {
              AlignmentError ae = new AlignmentError(ac, f, "diffAttributes");
              acErrors.add(ae);
              match = f;
            } else if ((ac.generateLabelling().equals(f.generateFlippedLabelling()))
                && ((f.getMin() != ac.getMin()) || (f.getMax() != ac.getMax()))) {
    ...
    for (AlignmentConstraint acUM : ac2) {
      for (AlignmentConstraint f2 : ac1f) {
        if ((acUM.node1.getXpath().equals(f2.node2.getXpath()))
            && (acUM.node2.getXpath().equals(f2.node1.getXpath()))) {
          if ((f2.getMin() == acUM.getMin())
              && (f2.getMax() == acUM.getMax())
              && (acUM.generateLabelling().equals(f2.generateFlippedLabelling()))) {
            match2 = f2;
          } else if ((f2.getMin() == acUM.getMin())
              && (f2.getMax() == acUM.getMax())
              && (!acUM.generateLabelling().equals(f2.generateFlippedLabelling()))) {
            AlignmentError ae = new AlignmentError(acUM, f2, "diffAttributes");
            acErrors.add(ae);
            match2 = f2;
          } else if ((acUM.generateLabelling().equals(f2.generateFlippedLabelling()))
              && ((f2.getMin() != acUM.getMin()) || (f2.getMax() != acUM.getMax()))) {
  ...
  //        ArrayList<WidthConstraint> wc1 = new ArrayList<WidthConstraint>(), wc2 = new
  // ArrayList<WidthConstraint>();
  //
  ...
  //        }
  //
  //        HashMap<WidthConstraint, WidthConstraint> matchedConstraints = new
  // HashMap<WidthConstraint, WidthConstraint>();
  ...
  //                if ( (wc.getPercentage() == temp.getPercentage()) && (wc.getAdjustment() ==
  // temp.getAdjustment()) && (wc.getMin() == temp.getMin()) && (wc.getMax() == temp.getMax())) {
  //                    match = temp;
  //                } else if ( ((wc.getMin()==temp.getMin()) && (wc.getMax()==temp.getMax())) && (
  // (wc.getPercentage()!=temp.getPercentage()) || (wc.getAdjustment()!=temp.getAdjustment()) ) ) {
  ...
  /**
   * Writes the model differences between the oracle and test versions into a text file, and then
   * opens the file in the default program of the user's system.
   *
          ...
          if (isErrorUnseen(e, defaultWidths)) {
            ...
            if (!e.generateKey().equals(previousKey)) {
              ...
              if (e.getOracle() != null) {
                ...
                output.append(
                    "\n"
                        + e.getOracle().node1.getXpath()
                        + type
                        + e.getOracle().node2.getXpath()
                        + "\n");
              } else {
                ...
                output.append(
                    "\n"
                        + e.getTest().node1.getXpath()
                        + type
                        + e.getTest().node2.getXpath()
                        + "\n");
              }
  ...
  public boolean foundDiffs() {
    return (vcErrors.size() != 0) || (acErrors.size() != 0);
  }
