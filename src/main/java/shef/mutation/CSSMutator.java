...
public class CSSMutator {
  ...
  public CSSMutator(
      String baseURL,
      String sh,
      LinkedHashMap<String, StyleSheet> stylesheets2,
      ArrayList<RuleSet> ruleCandidates,
      ArrayList<RuleMedia> mqCandidates2,
      Document page,
      int mutantNumber) {
    ...
    while (!madeMutant) {
      try {
        ...
        if (selector == 3) {
          ...
          changeCondition(me);
          //                    writeMutantToFile(i, "Changed expression to " + me.getFeature() + "
          // in \n" + rm);
          madeMutant = true;
        }
      } catch (Exception e) {
  ...
  private void mutateRule(
      HashMap<String, StyleSheet> sss, RuleSet rs, Declaration dec, int termIndex, int i) {
    ...
    if ((!property.equals("display"))
        && (!property.equals("position"))
        && (!property.equals("float"))) {
      changeRuleValueNumeric(
          property, t, true, true, i, ruleSet, toPrint, candidateDec, candidateTerm, i);
    } else {
      changeRuleValueFixed(
          property, t, true, true, i, ruleSet, toPrint, candidateDec, candidateTerm, i);
    }
  }

  private Declaration getCandidateFromSet(
      Declaration dec, ArrayList<RuleSet> newRuleCandidates, RuleSet ruleSet) {
  ...
  public void mutateRandomRule(
      int i, HashMap<String, StyleSheet> toMutate2, int selector2, int toggle) {
    ...
    while (!mutatedARule) {
      try {
        ...
        if (controller <= 3) {
          ...
          toMutate = getRuleSetFromRuleMedia(toMutateRM, selectedPruned);
        }
        ...
        if (selector2 == 0) {
          ...
          //                            changeRuleValue(property, t, mutatedARule, madeMutant, i,
          // toMutate, toPrint, decToMutate, 0);
          ...
          //                            System.out.println("Removed " + decToMutate + " from " +
          // decs.toString());
        }
        mutatedARule = true;
  ...
  private void changeRuleUnit(
      Term t,
      boolean mutatedARule,
      boolean madeMutant,
      HashMap<String, StyleSheet> toMutate2,
      List<Term<?>> terms,
      int selector,
      int i,
      Declaration decToMutate) {
  ...
  private void changeRuleValueNumeric(
      String property,
      Term t,
      boolean mutatedARule,
      boolean madeMutant,
    ...
    if ((!property.equals("display"))
      ...
      writeMutantToFile(
          i, "Changed term from " + orig + " to " + mutated + "\n" + toPrint + "\n" + toMutate);
  ...
  private void changeRuleValueFixed(
      String property,
      Term t,
      boolean mutatedARule,
      boolean madeMutant,
    ...
    t.setValue(newValue);
    writeMutantToFile(
        i, "Changed term from " + current + " to " + newValue + "\n" + toPrint + "\n" + toMutate);
    mutantDesc =
        "Changed term from " + current + " to " + newValue + "\n" + toPrint + "\n" + toMutate;
  ...
  @SuppressWarnings("unchecked")
  private TermNumericImpl<Float> getTermObject(
      HashMap<String, StyleSheet> toMutate2, boolean needTermLengthObject) {
    ...
    for (StyleSheet ss : toMutate2.values()) {
      for (RuleBlock rb : ss.asList()) {
        if (rb instanceof RuleSet) {
          ...
          //            		for (RuleSet rs : sets) {
          //            			ArrayList<Declaration> decs = (ArrayList<Declaration>)
          // ((RuleSet)rb).asList();
          //                		for (Declaration d : decs) {
          //                			ArrayList<Term<?>> terms = (ArrayList<Term<?>>) d.asList();
          //                			for (Term t : terms) {
  ...
  public void writeToFile(
      int counter, HashMap<String, StyleSheet> toMutate, String shorthand, String newUrl) {
    PrintWriter output = null;
    for (String s : toMutate.keySet()) {
      try {
        if (newUrl == null) {
          String dirName =
              preamble.replace("file:///", "/") + shorthand + "/mutant" + counter + "/resources/";
          File theDir =
              new File(
                  preamble.replace("file:///", "/")
                      + shorthand
                      + "/mutant"
                      + counter
                      + "/resources/");
  ...
  public void writeNewHtml(HashMap<String, StyleSheet> toMutate) {
    ...
    try {
      File theDir =
          new File(
              preamble.replace("file:///", "/") + shorthand + "/mutant" + this.mutantNumber + "/");
      boolean result = false;
      String dirName =
          preamble.replace("file:///", "/") + shorthand + "/mutant" + this.mutantNumber + "/";
      if (!theDir.exists()) {
  ...
  public RuleBlock getRuleBlockFromStyleSheet(
      HashMap<String, StyleSheet> toMutate2, RuleBlock block) {
      ...
      for (StyleSheet ss : toMutate2.values()) {
        for (RuleBlock rb : ss.asList()) {
          if (rb instanceof RuleMedia) {
            if (rb.equals(block)) {
              matches.add(rb);
              //					} else if (((RuleMedia) rb).getMediaQueries().equals(((RuleMedia)
              // block).getMediaQueries())) {
  ...
  private boolean haveSameMediaQueries(RuleMedia rb, RuleMedia block) {
    ...
    if (mq1.size() != mq2.size()) {
      ...
      return mq1.toString().equals(mq2.toString());
    }
  }
  ...
  public HashMap<String, LinkedHashMap<String, StyleSheet>> getMutationOptions(
      LinkedHashMap<String, StyleSheet> currentSS) {
    ...
    for (RuleSet rs : regCanCloned) {
      for (Declaration dec : rs) {
        ...
        if ((!property.equals("display"))
          ...
          for (int t = 0; t < dec.asList().size(); t++) {
            ...
            mutatedCM =
                new CSSMutator(
                    baseURL,
                    shorthand,
                    (LinkedHashMap<String, StyleSheet>) toMutate,
                    regCandidates,
                    mqCandidates,
                    htmlDoc,
                    0);
          ...
          //                            toMutate = cloner.deepClone(currentSS);
          //                            mutateRule(toMutate, rs, dec, 0, d);
          //                            mutated.put(mutantDesc, (LinkedHashMap<String, StyleSheet>)
          // toMutate);
          //                        }
          //                    } else if (property.equals("position")) {
          //                        for (int d = 0; d < positionOptions.length; d++) {
