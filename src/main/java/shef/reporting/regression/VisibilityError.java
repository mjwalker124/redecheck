...
public class VisibilityError extends Error {
  ...
  public ArrayList<int[]> calculateRangeOfViewportWidths() {
    ...
    if ((vco.getAppear() != vct.getAppear()) && (vco.getDisappear() == vct.getDisappear())) {
      errorRanges.add(
          new int[] {
            Math.min(vco.getAppear(), vct.getAppear()),
            Math.max(vco.getAppear(), vct.getAppear()) - 1
          });
    } else if ((vco.getAppear() == vct.getAppear()) && (vco.getDisappear() != vct.getDisappear())) {
      errorRanges.add(
          new int[] {
            Math.min(vco.getDisappear(), vct.getDisappear()) + 1,
            Math.max(vco.getDisappear(), vct.getDisappear())
          });
    } else {
      errorRanges.add(
          new int[] {
