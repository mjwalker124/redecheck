...
public class RLGAnalyser {
  ...
  private void filterOutDuplicateReports() {
    ...
    ArrayList<ResponsiveLayoutFailure> cloned1 =
        (ArrayList<ResponsiveLayoutFailure>) errors.clone();
    ...
    ArrayList<ResponsiveLayoutFailure> cloned2 =
        (ArrayList<ResponsiveLayoutFailure>) errors.clone();
  ...
  public void checkForViewportOverflows(HashMap<String, Node> nodes) {
    ...
    for (Node n : nodes.values()) {
      ...
      if (!n.getXpath().equals("/HTML/BODY") && !n.getXpath().contains("HEAD")) {
        ...
        // Checks that at least one parent constraint exists, as if not the element is always
        // outside the viewport and therefore
  ...
  /**
   * This method examines the alignment constraints from the RLG under test to see if any
   * overlapping or overflowing elements have been found
   *
   * @param alignmentConstraints the set of constraints to analyse
   */
  public void detectOverflowOrOverlap(
    ...
    for (AlignmentConstraint ac : alignmentConstraints.values()) {
      ...
      if (ac.getType() == Type.SIBLING) {
        ...
        if (ac.getAttributes()[10]) {
          ...
          // Only continue checking if the current constraint was not identified as a collision
          // failure
          if (!collision) {
  ...
  /**
   * This method traverses the RLG to obtain a set of ancestors for a node at a given viewport
   * width.
   *
  ...
  //                    if ((ac1.getXpath().equals(overflowed.getXpath()) ||
  // ac2.getXpath().equals(overflowed.getXpath()))) {
  ////                        System.out.println("Wanted to remove " + ac + " because of " +
  // overflowed.getXpath());
  ...
  /**
   * This method analyses the constraints to see if any represent potential small-range layout
   * inconsistencies
   *
   * @param alignmentConstraints the set of constraints to analyse
   */
  public void checkForSmallRanges(
      HashBasedTable<String, int[], AlignmentConstraint> alignmentConstraints) {
    for (AlignmentConstraint ac : alignmentConstraints.values()) {
   ...
   */
  private AlignmentConstraint getPreviousOrNextConstraint(
      AlignmentConstraint ac, boolean i, boolean matchType) {
    ...
    for (AlignmentConstraint con : rlg.getAlignmentConstraints().values()) {
      ...
      if ((ac1xp.equals(con1xp) && ac2xp.equals(con2xp))
          || (ac1xp.equals(con2xp) && ac2xp.equals(con1xp))) {
        if (i) {
  ...
  //    private boolean parentAlsoOverflowed(Node overflowed, ArrayList<ResponsiveLayoutFailure>
  // cloned) {
  ...
  /**
   * Checks each node in turn and looks whether any of its children elements wrap incongruously at
   * any point
   */
  private void checkForWrappingElements() {
    ...
    for (Node n : rlg.getNodes().values()) {
      ...
      if (children.size() > 1) {
        ...
        HashMap<String, HashMap<String, ArrayList<AlignmentConstraint>>> totalRowCons =
            new HashMap<>();
            ...
            for (AlignmentConstraint ac : fullSet) {
              ...
              if (alignedInRow(ac)) {
                ...
                if (match == null) {
                  ...
                  rows.add(newRow);
                  rowSibConstraints.put(
                      setOfNodesToString(newRow), new ArrayList<AlignmentConstraint>());
            ...
            for (AlignmentConstraint acOV : overlapping) {
              ...
              for (ArrayList<Node> row : clonedRows) {
                if (row.contains(acOV.getNode1()) && row.contains(acOV.getNode2())) {
                  ...
                  removeConstraintsFromCol(
                      acOV.getNode1(), acOV.getNode2(), rowSibConstraints, row);
                }
              }
            }
            ...
            if (rows.size() > 0) {
              ...
              if (nonWrappedRow != null) {
                ...
                if ((nonWrappedRow.size() - wrappedRow.size() == 1)
                    || (children.size() - wrappedRow.size() == 1)) {
                      ...
                      if (elementNowBelowRow(notInRow, nonWrappedRow, grouped.get(key))) {
                        ...
                        WrappingFailure we =
                            new WrappingFailure(
                                notInRow,
                                nonWrappedRow,
                                getNumberFromKey(key, 0),
                                getNumberFromKey(key, 1));
                        errors.add(we);
                      }
                    }
                  }
   ...
   */
  private ArrayList<Node> getWrappedRow(
      ArrayList<ArrayList<Node>> rows, ArrayList<Node> nonWrappedRow) {
    // Iterate through
  ...
  /**
   * This method inspects the alignment constraints to see whether the supposedly 'wrapped' element
   * is now below the rest of the row
   *
   ...
   */
  private boolean elementNowBelowRow(
      Node notInRow, ArrayList<Node> nonWrappedRow, HashSet<AlignmentConstraint> grouped) {
    ...
    for (AlignmentConstraint ac : grouped) {
      ...
      if ((n1.getXpath().equals(notInRow.getXpath()) && nonWrappedRow.contains(n2))
        ...
        // Check the alignment attributes and return true if the wrapped element is below the
        // non-wrapped one.
  ...
  /**
   * This method checks whether the element supposedly 'wrapped' is in a row at the next behaviour
   * range and returns it if it is.
   *
   * @param notInRow The node no longer in a row
   ...
   */
  private ArrayList<Node> getRowInNextRange(
      Node notInRow, HashMap<String, ArrayList<ArrayList<Node>>> rows, String key) {
  ...
  //                for (AlignmentConstraint ac : rlg.getAlignmentConstraints().values()) {
  //                    if (ac.getType() == Type.SIBLING) {
  //                        if ((children.contains(ac.getNode1())) &&
  // (children.contains(ac.getNode2()))) {
  //                            if (childrenWithinParent(ac, n)) {
  //                                sibs.add(ac);
  //                                values.add(ac.getMin());
  //                                values.add(ac.getMax());
  ...
  //                HashMap<String, HashMap<String, ArrayList<AlignmentConstraint>>> totalRowCons =
  // new HashMap<>();
  ////                HashMap<String, HashMap<String, ArrayList<AlignmentConstraint>>> totalColCons
  // = new HashMap<>();
  //                HashMap<String, HashSet<Node>> nodesInParentMap = new HashMap<>();
  //
  //
  ...
  //                        HashMap<String, ArrayList<AlignmentConstraint>> rowSibConstraints = new
  // HashMap<>();
  //                        HashMap<String, ArrayList<AlignmentConstraint>> rowPCConstraints = new
  // HashMap<>();
  //
  ////                        HashMap<String, ArrayList<AlignmentConstraint>> colSibConstraints =
  // new HashMap<>();
  ////                        HashMap<String, ArrayList<AlignmentConstraint>> colPCConstraints = new
  // HashMap<>();
  //                        ArrayList<Node> nodesNotInRows = (ArrayList<Node>) children.clone();
  ////                        ArrayList<Node> nodesNotInColumns = (ArrayList<Node>)
  // children.clone();
  //                        HashSet<AlignmentConstraint> fullSet = grouped.get(key);
  //                        HashSet<AlignmentConstraint> overlapping = new HashSet<>();
  //
  ...
  //
  //                            if (toggle != 0) {
  //                                ArrayList<Node> match = matchingExistingPattern(ac, rows,
  // columns, toggle, fullSet);
  //                                if (match == null) {
  ...
  //                                    if (toggle == 1) {
  //                                        rows.add(newRowCol);
  //                                        rowSibConstraints.put(setOfNodesToString(newRowCol), new
  // ArrayList<AlignmentConstraint>());
  //
  // rowSibConstraints.get(setOfNodesToString(newRowCol)).add(ac);
  //                                    } else {
  ////                                        columns.add(newRowCol);
  ////                                        colSibConstraints.put(setOfNodesToString(newRowCol),
  // new ArrayList<AlignmentConstraint>());
  ////
  ...
  //                                    if (!match.contains(ac.getNode1())) {
  //                                        match.add(ac.getNode1());
  ////                                        System.out.println(ac.getNode1().getXpath() + " added
  // to " + matchKey + "  " + match.size());
  //                                    } else if (!match.contains(ac.getNode2())) {
  //                                        match.add(ac.getNode2());
  ////                                        System.out.println(ac.getNode2().getXpath() + " added
  // to " + matchKey + "  " + match.size());
  //                                    }
  //                                    if (toggle == 1) {
  //                                        ArrayList<AlignmentConstraint> cons =
  // rowSibConstraints.get(matchKey);
  //                                        rowSibConstraints.remove(matchKey);
  //                                        rowSibConstraints.put(setOfNodesToString(match), cons);
  //
  // rowSibConstraints.get(setOfNodesToString(match)).add(ac);
  //                                    } else {
  ////                                        ArrayList<AlignmentConstraint> cons =
  // colSibConstraints.get(matchKey);
  ////                                        colSibConstraints.remove(matchKey);
  ////                                        colSibConstraints.put(setOfNodesToString(match),
  // cons);
  ////
  ...
  //                                }
  //
  //                                // Remove from ArrayLists to detect elements not placed into row
  // or column
  //                                if (toggle == 1) {
  //                                    nodesNotInRows.remove(ac.getNode1());
  //                                    nodesNotInRows.remove(ac.getNode2());
  ...
  //
  //                            // See if the elements were in a row or column
  //                            ArrayList<ArrayList<Node>> clonedRows = (ArrayList<ArrayList<Node>>)
  // rows.clone();
  //                            for (ArrayList<Node> row : clonedRows) {
  //                                if (row.contains(acOV.getNode1()) &&
  // row.contains(acOV.getNode2())) {
  //                                    ArrayList<Node> actualrow =
  // rows.get(clonedRows.indexOf(row));
  //                                    actualrow.remove(acOV.getNode1());
  //                                    actualrow.remove(acOV.getNode2());
  //                                    removeConstraintsFromCol(acOV.getNode1(), acOV.getNode2(),
  // rowSibConstraints, row);
  //                                }
  //                            }
  //                        }
  ...
  //                    ArrayList<ArrayList<Node>> rows = totalRows.get(key);
  //                    ArrayList<Node> not = totalNotInRows.get(key);
  //                    HashMap<String, ArrayList<AlignmentConstraint>> consRow =
  // totalRowCons.get(key);
  //                    for (Node notInRow : not) {
  //
  //                        // Need to refine this to the entire row becoming a column!
  //                        if (rows.size() > 0) {
  //                            ArrayList<Node> nonWrappedRow = inRowInNextRange(notInRow,
  // totalRows, key);
  //
  //                            if (nonWrappedRow != null) {
  //                                ArrayList<Node> wrappedRow = getWrappedRow(rows, nonWrappedRow);
  ...
  //                                        if (elementStillWithinParent(notInRow, n, key)) {
  //                                            if (elementNowBelowRow(notInRow, rows,
  // nonWrappedRow, grouped.get(key))) {
  //                                                WrappingFailure we = new
  // WrappingFailure(notInRow, nonWrappedRow, getNumberFromKey(key, 0), getNumberFromKey(key, 1));
  //                                                errors.add(we);
  //                                            }
  //                                        }
  //                                    }
  ...
  //    }
  //
  //    private ArrayList<Node> getWrappedRow(ArrayList<ArrayList<Node>> rows, ArrayList<Node>
  // nonWrappedRow) {
  //        for (Node n : nonWrappedRow) {
  //            for (ArrayList<Node> row : rows) {
  //                if (row.contains(n)) {
  ...
  //    }
  //
  //    private boolean elementNowBelowRow(Node notInRow, ArrayList<ArrayList<Node>> rows,
  // ArrayList<Node> nonWrappedRow, HashSet<AlignmentConstraint> grouped) {
  //
  //        for (AlignmentConstraint ac : grouped) {
  //            Node n1 = ac.getNode1();
  //            Node n2 = ac.getNode2();
  //
  //            if ((n1.getXpath().equals(notInRow.getXpath()) && nonWrappedRow.contains(n2))
  //                    || (n2.getXpath().equals(notInRow.getXpath()) &&
  // nonWrappedRow.contains(n1))) {
  ...
  //    }
  //
  //    private ArrayList<Node> inRowInNextRange(Node notInRow, HashMap<String,
  // ArrayList<ArrayList<Node>>> rows, String key) {
  //        int x = getNumberFromKey(key,1);
  //        for (String key2 : rows.keySet()) {
  //            if (!key.equals(key2)) {
  //                int first = getNumberFromKey(key2, 0);
  ...
  //    }
  //
  //    private ArrayList<Node> matchingExistingPattern(AlignmentConstraint ac,
  // ArrayList<ArrayList<Node>> rows, ArrayList<ArrayList<Node>> columns, int rowCol,
  // HashSet<AlignmentConstraint> fullSet) {
  //        Node n1 = ac.getNode1();
  //        Node n2 = ac.getNode2();
  //
  ...
  private void removeConstraintsFromCol(
      Node f1,
      Node f2,
      HashMap<String, ArrayList<AlignmentConstraint>> colSibConstraints,
      ArrayList<Node> newCol) {
    HashMap<String, ArrayList<AlignmentConstraint>> clonedMap =
        (HashMap<String, ArrayList<AlignmentConstraint>>) colSibConstraints.clone();
    for (String s : clonedMap.keySet()) {
      if (s.contains(f1.getXpath()) && s.contains(f2.getXpath())) {
        ...
        for (AlignmentConstraint con : temp) {
          if (con.getNode1().getXpath().equals(f1.getXpath())
              || (con.getNode1().getXpath().equals(f2.getXpath()))) {
            cons.remove(con);
          } else if (con.getNode2().getXpath().equals(f1.getXpath())
              || (con.getNode2().getXpath().equals(f2.getXpath()))) {
            cons.remove(con);
          }
        }
  ...
  private boolean elementVisible(Node wrapped, String key) {
    ...
    return (nodeMax >= min) && (nodeMin <= max);
  }
   ...
   */
  private void putConstraintIntoGroups(
      HashMap<String, HashSet<AlignmentConstraint>> grouped, AlignmentConstraint ac) {
  ...
  /**
   * This method analyses the existing rows extracted to see if the current constraint matches any
   * of them
   *
   ...
   */
  private ArrayList<Node> getMatchingExistingRow(
  ...
  private ArrayList<AlignmentConstraint> getSiblingEdges(
      Node n1, HashSet<AlignmentConstraint> fullSet) {
